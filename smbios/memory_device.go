...
func NewMemoryDevice(s *smbios.Structure) *MemoryDevice {
	return &MemoryDevice{
		...
		DeviceSet:                    GetDeviceSet(s, 0x0F),
		DeviceLocator:                *GetString(s, 0x10),
		BankLocator:                  GetString(s, 0x11),
		MemoryType:                   MemoryType(GetByte(s, 0x12)),
		TypeDetail:                   TypeDetail(GetByte(s, 0x13)),
		Speed:                        MemoryDeviceSpeed(GetWord(s, 0x15)),
		Manufacturer:                 GetString(s, 0x17),
		SerialNumber:                 GetString(s, 0x18),
		AssetTag:                     GetString(s, 0x19),
		PartNumber:                   GetString(s, 0x1A),
...
func GetDeviceSet(s *smbios.Structure, offset int) *string {
	b := GetByte(s, offset)
