...
// array extended maximum capacity, in bytes, for this array.
...
func getCapacity(maximumCapacity uint32, extendedMaximumCapacityOffset uint64) string {
	var bytes uint64
	if maximumCapacity < 0x80000000 {
		bytes = uint64(maximumCapacity)
	} else {
		bytes = extendedMaximumCapacityOffset
	}
